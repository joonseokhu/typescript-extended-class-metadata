...
import {
  ClassPropertyMetadata, getPropertyMetadata, getPropertyNames, ValueTypeName,
} from '../../dist/lib';
...
@UseRichMeta((prop, target, applyDecorators) => {
  if (prop.type === ValueTypeName.Number) {
...
const Inject = (): ParameterDecorator => (target, propertyKey) => {};

export class MethodTest extends FooType('hi') {
  constructor(
    @Inject() private a: number,
    public b: boolean[],
  ) {
    super();
  }
  ...
  getFoo1(a: string, b = 1, d?: string): Foo {
    return new Foo();
  }
